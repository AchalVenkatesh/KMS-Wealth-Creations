...
import (
	...
	// "example.com/KMS-trading/templates"
...
func main(){
	...
	auth.GET("/dashboard",func(c *gin.Context){
		elite,_:=c.Cookie("elite")
		if elite=="true"{
			c.HTML(http.StatusOK,"elite-dashboard.html","")
			return
		}
	...
	auth.GET("/pastPosts",func(c *gin.Context){
		elite,_:=c.Cookie("elite")
		if elite=="true"{
			c.HTML(http.StatusOK,"elite-past-targets.html","")
			return
		}
	...
	auth.GET("/news",func(c *gin.Context){
		elite,_:=c.Cookie("elite")
		if elite=="true"{
			c.HTML(http.StatusOK,"elite-news.html","")
			return
		}
		c.HTML(http.StatusOK,"news.html","")
	})
	auth.GET("/profile",userProfile(ctx,client))
	auth.GET("/posts",getPosts(ctx,client))
	auth.GET("/elite",elitePosts(ctx,client))
	auth.GET("/recent-updates",recentUpdates(ctx,client))
	...
	admin.GET("/elite",getAdminElite(ctx,client))
	...
	admin.DELETE("/deleteElitePosts",deleteElitePosts(ctx,client))
...
func adminLogin(ctx context.Context,client *db.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		...
		fmt.Println("Stored Password:",stored_password)
...
func postPosts(ctx context.Context, client *db.Client) gin.HandlerFunc{
    return func(c *gin.Context){
		...
		if !lastRequestTime.IsZero() && (lastRequestTime.Format("2006-01-02") != now.Format("2006-01-02")){
			var posts map[string]Posts
			ref:=client.NewRef("server/saving-data/fireblog")
		// username
		userRef := client.NewRef("server/saving-data/fireblog/recent-updates")
		...
		fn := func(tn db.TransactionNode) (interface{}, error) {
			var allPosts map[string]Posts
			if err := tn.Unmarshal(&allPosts); err != nil {
				return nil, err
			}
		
			if allPosts == nil {
				allPosts = make(map[string]Posts)
			}
		...
		err := ref.Child("posts").Transaction(ctx, fn)
		if err != nil {
			log.Println("Error updating posts data: ", err)
		}
        ...
        ref := client.NewRef("server/saving-data/fireblog/recent-updates")
		newPost := Posts{
            Comments:      comments,
...
func getPosts(ctx context.Context, client *db.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		eliteRef := client.NewRef("sever/saving-data/fireblog/elite_posts")
		if err := eliteRef.Get(ctx, &elitePosts);err!=nil{
			log.Printf("error getting elite data: %v",err)
...
func userProfile(ctx context.Context, client *db.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		var user Users
		// Hey:="RELIANCE"
		...
		if err!=nil {
			...
			c.String(http.StatusInternalServerError,"There was some error while authenticating. Please try again.") // or userID.(int), depending on the type
			// Use userIDString
		}

		elite,err := c.Cookie("elite")
		if err!=nil{
			fmt.Println("Not elite")
			elite="false"
		}
		fmt.Println("UserID from claims:",userID)
		ref:=fmt.Sprintf("server/saving-data/fireblog/users/%s",userID)
		userRef := client.NewRef(ref)
		if err := userRef.Get(ctx, &user); err != nil {
    		log.Fatalf("error getting user data: %v", err)
		}
		log.Print(user)
		r := gintemplrenderer.New(c.Request.Context(), http.StatusOK,ProfilePage(user,elite))
		c.Render(http.StatusOK, r)
	}
}

func recentUpdates(ctx context.Context, client *db.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		var posts map[string]Posts
		// username
		elite,err:=c.Cookie("elite")
		...
		r := gintemplrenderer.New(c.Request.Context(), http.StatusOK,RecentUpdates(posts,elite))
		c.Render(http.StatusOK, r)
    }
}

func elitePosts(ctx context.Context, client *db.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		var posts map[string]Posts
		// username
		elite,err:=c.Cookie("elite")
		...
		userRef := client.NewRef("server/saving-data/fireblog/elite-posts")
		if err := userRef.Get(ctx, &posts); err != nil {
			log.Printf("error getting user data: %v", err)
			c.String(http.StatusInternalServerError, "Error fetching posts")
			return
		...
		r := gintemplrenderer.New(c.Request.Context(), http.StatusOK,ElitePosts(posts,elite))
		c.Render(http.StatusOK, r)		
	}
}

func getAdminElite(ctx context.Context, client *db.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		var posts map[string]Posts
		// Hey:="RELIANCE"
		userRef := client.NewRef("server/saving-data/fireblog/elite-posts")
		...
		log.Print(posts)
		r := gintemplrenderer.New(c.Request.Context(), http.StatusOK,AdminElites(posts))
		c.Render(http.StatusOK, r)
	}
}

func deleteElitePosts(ctx context.Context, client *db.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		id := c.Query("stock-name")
		fmt.Println(id)
		userRef := client.NewRef(fmt.Sprintf("server/saving-data/fireblog/elite-posts/%s",id))
		fmt.Println(userRef)
		if err := userRef.Delete(ctx); err != nil {
			log.Fatalf("error deleting user data: %v", err)
		}
